...
def test_worker_config_points_to_app_assets_and_worker():
    raw = (ROOT / "wrangler.worker.jsonc").read_text(encoding="utf-8")
...
def test_package_has_cloudflare_scripts():
    ...
    assert scripts.get("cf:dev") == "wrangler dev -c wrangler.worker.jsonc"
    assert scripts.get("cf:deploy") == "wrangler deploy -c wrangler.worker.jsonc"
    assert scripts.get("cf:dry") == "wrangler deploy --dry-run -c wrangler.worker.jsonc"
...
def test_worker_keeps_risky_routes_disabled_by_default():
    ...
    assert "Cloudflare AI is disabled" in worker
