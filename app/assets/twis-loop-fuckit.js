...
function install(){if(installed)return;const host=q('#twisLoopDeck');if(!host){setTimeout(install,100);return}installed=true;css();const wrap=document.createElement('div');wrap.className='twis-fuck-wrap';wrap.innerHTML='<button class="twis-fuck-main" id="twisBigFuck">FUCK IT — BUILD ME SOMETHING</button><button class="twis-fuck-undo" id="twisBigUndo">UNDO</button>';const stEl=q('#ldStatus');host.insertBefore(wrap,stEl||null);q('#twisBigFuck').onclick=build;q('#twisBigUndo').onclick=restore;window.TWIS_FUCK_IT={build,restore}}
